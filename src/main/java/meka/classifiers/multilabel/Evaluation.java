...
public class Evaluation {
   ...
   */
  public static void runExperiment(MultiLabelClassifier h, final String options[]) throws Exception {
    ...
    try {
      ...
      if (Utils.getOptionPos('x', options) >= 0) {
        ...
        if (h.getDebug()) {
          System.out.println(":- Dataset -: " + MLUtils.getDatasetName(D_train) + "\tL=" + L + "\tD(t:T)=(" + D_train.numInstances() + ":" + D_test.numInstances() + ")\tLC(t:T)="
              + Utils.roundDouble(MLUtils.labelCardinality(D_train, L), 2) + ":" + Utils.roundDouble(MLUtils.labelCardinality(D_test, L), 2) + ")");
        }
        ...
        if (D_train.numInstances() > 0 && D_test.numInstances() > 0 && r != null) {
          System.out.println(r.toString());
        }
   ...
   */
  public static boolean isMT(final Instances D) {
    int L = D.classIndex();
    for (int j = 0; j < L; j++) {
  ...
  /**
   * EvaluateModel - Build model 'h' on 'D_train', test it on 'D_test', threshold it according to
   * 'top', using default verbosity option.
   *
   * @param h
   *          a multi-dim. classifier
   * @param D_train
   *          training data
   * @param D_test
   *          test data
   * @param top
   *          Threshold OPtion (pertains to multi-label data only)
   * @return Result raw prediction data with evaluation statistics included.
   */
  public static Result evaluateModel(final MultiXClassifier h, final Instances D_train, final Instances D_test, final String top) throws Exception {
   ...
   *          Threshold OPtion (pertains to multi-label data only)
   * @param vop
   *          Verbosity OPtion (which measures do we want to calculate/output)
   * @return Result raw prediction data with evaluation statistics included.
   */
  public static Result evaluateModel(final MultiXClassifier h, final Instances D_train, final Instances D_test, final String top, final String vop) throws Exception {
  ...
  /**
   * EvaluateModel - Assume 'h' is already built, test it on 'D_test', threshold it according to
   * 'top', verbosity 'vop'.
   *
   * @param h
   *          a multi-dim. classifier
   ...
   *          test data
   * @param tal
   *          Threshold VALUES (not option)
   * @param vop
   *          Verbosity OPtion (which measures do we want to calculate/output)
   * @return Result raw prediction data with evaluation statistics included.
   */
  public static Result evaluateModel(final MultiXClassifier h, final Instances D_test, final String tal, final String vop) throws Exception {
   ...
   * @return Result raw prediction data with evaluation statistics included.
   */
  public static Result cvModel(final MultiLabelClassifier h, final Instances D, final int numFolds, final String top) throws Exception {
   ...
   * @return Result raw prediction data with evaluation statistics included.
   */
  public static Result cvModel(final MultiLabelClassifier h, final Instances D, final int numFolds, final String top, final String vop) throws Exception {
    ...
    for (int i = 0; i < numFolds; i++) {
      ...
      if (h.getDebug()) {
        System.out.println(":- Fold [" + i + "/" + numFolds + "] -: " + MLUtils.getDatasetName(D) + "\tL=" + D.classIndex() + "\tD(t:T)=(" + D_train.numInstances() + ":"
            + D_test.numInstances() + ")\tLC(t:T)=" + Utils.roundDouble(MLUtils.labelCardinality(D_train, D.classIndex()), 2) + ":"
  ...
  /**
   * EvaluateModel - Build model 'h' on 'D_train', test it on 'D_test'. Note that raw multi-label
   * predictions returned in Result may not have been thresholded yet. However, data statistics,
   * classifier info, and running times are inpregnated into the Result here.
   *
   * @param h
   *          a multi-dim. classifier
   * @param D_train
   ...
   */
  public static Result evaluateModel(final MultiXClassifier h, final Instances D_train, final Instances D_test) throws Exception {
    ...
    // System.out.println(":- Classifier -: "+h.getClass().getName()+":
    // "+Arrays.toString(h.getOptions()));
  ...
  /*
   * allow threaded evaluation of model, all instances are passed to the classifier then they are
   * gathered in results, for short datasets the overhead might be significant
   */
  public static Result evaluateModelM(final MultiXClassifier h, final Instances D_train, final Instances D_test, final String top, final String vop) throws Exception {
    // Train
    long before = System.currentTimeMillis();
    /*
   ...
   *
   * @param h
   *          a multi-dim. classifier, ALREADY BUILT
   * @param D_test
   *          test data
   * @return Result with raw prediction data ONLY
   */
  public static Result testClassifier(final MultiXClassifier h, final Instances D_test) throws Exception {
     ...
     * System.out.println("\t"+Arrays.toString(result.rowTrue(i))+" vs "+Arrays.toString(result.
     * rowRanking(i))); } }
     */
   ...
   *
   * @param h
   *          a multi-dim. classifier, ALREADY BUILT (threaded, implements MultiLabelThreaded)
   * @param D_test
   *          test data
   * @return Result with raw prediction data ONLY
   */
  public static Result testClassifierM(final MultiXClassifier h, final Instances D_test) throws Exception {
    ...
    if (h instanceof MultiLabelClassifierThreaded) {
       ...
       *
       * for(int i = 0; i < result.size(); i++) {
       * System.out.println("\t"+Arrays.toString(result.rowActual(i))+" vs "+Arrays.toString(result.
       * rowRanking(i))); }
       *
       *
       * }
  ...
  /**
   * GetDataset - load a dataset, given command line options specifying an arff file, and set the
   * class index correctly to indicate the number of labels.
   *
   * @param options
   *          command line options
   * @param T
   *          set to 'T' if we want to load a test file
   * @return An Instances representing the dataset public static Instances getDataset(String
   *         options[], char T) throws Exception { Instances D = loadDataset(options, T);
   *         setClassesFromOptions(D,MLUtils.getDatasetOptions(D)); return D; }
   */
   ...
   *          command line options
   * @return An Instances representing the dataset public static Instances getDataset(String
   *         options[]) throws Exception { return getDataset(options,'t'); }
   */
   ...
   *
   * @param options
   *          command line options, specifying dataset filename
   * @return the dataset
   */
  public static Instances loadDataset(final String options[]) throws Exception {
   ...
   *          command line options, specifying dataset filename
   * @param T
   *          set to 'T' if we want to load a test file (default 't': load train or train-test file)
   * @return the dataset
   */
  public static Instances loadDataset(final String options[], final char T) throws Exception {
  ...
  /*
   * GetL - get number of labels (option 'C' from options 'options'). private static int getL(String
   * options[]) throws Exception { return (Utils.getOptionPos('C', options) >= 0) ?
   * Integer.parseInt(Utils.getOption('C',options)) : 0; }
   */

  /*
   * SetClassesFromOptions - set the class index correctly in a dataset 'D', given command line
   * options 'options'. <br> NOTE: there is a similar function in Exlorer.prepareData(D) but that
   * function can only take -C from the dataset options. <br> TODO: replace the call to
   * Exlorer.prepareData(D) with this method here (use the name 'prepareData' -- it souds better).
   * public static void setClassesFromOptions(Instances D, String options[]) throws Exception { try {
   * // get L int L = getL(options); // if negative, then invert first if ( L < 0) { L = -L; D =
   * F.mulan2meka(D,L); } // set L D.setClassIndex(L); } catch(Exception e) { e.printStackTrace();
   * throw new Exception
   * ("[Error] Failed to Set Classes from options. You must supply the number of labels either in the @Relation Name of the dataset or on the command line using the option: -C <num. labels>"
   * ); } }
   */

  public static void printOptions(final Enumeration e) {
